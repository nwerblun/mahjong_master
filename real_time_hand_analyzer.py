...
class HandAnalyzer(Frame):
    def __init__(self, master=None):
        ...
        self.auto_hand_vizualizer_frame = Frame(self.master, borderwidth=3, relief=GROOVE)
        self.app_select_frame.pack(expand=NO, side=TOP, fill=X)
    ...
    def _active_app_preview_loop(self):
        if win32gui.GetForegroundWindow() != self.selected_window:
            ...
            self.preview_callback_id = self.after(20, self._active_app_preview_loop)
            return
        else:
            ...
            img_with_boxes = get_pred_output_img(img, self.xy, self.wh, self.conf, self.cls,
                                                 class_thresh=0.95, conf_thresh=0.75, nms_iou_thresh=0.2)
            # TODO: Verify that you don't need to create a copy of the array before assigning
    ...
    def create_application_preview(self):
        ...
        self.preview_callback_id = self.after(20, self._active_app_preview_loop)
        self.preview_img_lbl.pack(side=TOP, expand=YES, fill=BOTH, anchor="center")
